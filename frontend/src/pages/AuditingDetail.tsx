  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const page = await getPageWithSections('auditing', slug);
        setDynamicPage(page);
        const firstSection = (page.sections && page.sections[0]) || null;
        setDynamicSection(firstSection);
        // Images
        const images = await imageService.getAuditingImages(slug);
        setCloudinaryImages(images);
      } catch (error) {
        console.error('Error fetching auditing detail:', error);
      } finally {
