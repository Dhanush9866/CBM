  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const page = await getPageWithSections('verification-certification', slug);
        setDynamicPage(page);
        const firstSection = (page.sections && page.sections[0]) || null;
        setDynamicSection(firstSection);
        // Images
        const images = await imageService.getVerificationCertificationImages(slug);
        setCloudinaryImages(images);
      } catch (error) {
        console.error('Error fetching verification & certification detail:', error);
      } finally {
